...
"""
way to manuplate tile map according to the postion of the respective piece
"""
            ...
            for col_ind, val in enumerate(row):
                ...
                if self.mouse_pos == [col_ind, row_ind]:
                    ...
                    if self.white_king.rect.x==x and self.white_king.rect.y == y:
                        self.selected_or_not = True
                        self.selected_piece = {
                                "p": self.white_king, "name": "r_w_1", "pos": (x, y)}
                    if self.white_queen.rect.x==x and self.white_queen.rect.y == y:
                        self.selected_or_not = True
                        self.selected_piece = {"p":self.white_queen,"pos":(x,y)}
                    ...
                    if self.black_king.rect.x==x and self.black_king.rect.y == y:
                        self.selected_or_not = True
                        self.selected_piece = {
                                "p": self.black_king, "name": "r_w_1", "pos": (x, y)}
                    if self.black_queen.rect.x==x and self.black_queen.rect.y == y:
                        self.selected_or_not = True
                        self.selected_piece = {"p":self.black_queen,"pos":(x,y)}

    def move_pieces(self,pos):
        self.piece = self.selected_piece["p"]
        self.piece.pos = [pos[0]*64+128,pos[1]*64+128]
        self.selected_or_not = False
...
#TODO
#FIX KING AND QUEEN MOVEMENTS
# CHECK WHETHER THEY ARE BEING SELECTED IN SLECET PIECE FUNCTION
